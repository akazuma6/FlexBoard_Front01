...
const AuthContext = createContext({
	...
	authToken: null,
	loginUser: async () => {
		console.error("loginUser関数が呼び出されました");
	},
	logoutUser: () => {
		console.error("logoutUser関数が呼び出されました");
	},
});
	...
	const loginUser = async (username, password) => {
		...
		try {
			...
			if (response.status === 200) {
				...
				console.log("Decoded JWT:", decodedUser); // Log decoded token for debugging
				setUser(decodedUser);
				localStorage.setItem("authToken", JSON.stringify(data));
				setLoading(false);
				return decodedUser; // ユーザーオブジェクトを返す
			}
			setLoading(false);
			return null; // ログイン失敗
		} catch (error) {
			...
			alert(
				"ログインに失敗しました。従業員番号とパスワードを確認してください。",
			);
			setLoading(false);
			return null; // ログイン失敗
		}
